...
# 블록  
블록이 불리언 값을 입력으로 요구하면, 입력이 없으면 일반적으로 **거짓**으로 해석됩니다. 아래에 예시가 제공됩니다.  
불리언 값이 기대되는 곳에 비불리언 값을 직접 연결할 수는 없지만, 비불리언 값을 변수에 저장한 뒤 그것을 입력으로 전달하는 것은 가능합니다.  
...
일상적인 말처럼, 블록 컴포저에서 리스트는 "할 일 목록"이나 "쇼핑 목록"처럼 **항목들이 순서대로 나열된 집합**입니다.  
