...
한번 바퀴 속도를 설정하면, 다시 바퀴 속도를 설정하기 전까지 해당 속도로 삐오가 이동합니다.
...
## 바퀴가 움직이는 중인가?
삐오의 바퀴가 움직이고 있는지 아닌지 여부를 **참(1) / 거짓(0)** (으)로 반환합니다.
...
### 자바스크립트 코드
```javascript
// 삐오 목 각도를 10 도로 바꾸기 | 기다리기 O
$('Pio*0:neck.angle').d = 10;
await $('Pio*0:neck.!angle').w();

// 삐오 목 각도를 -10 도로 바꾸기 | 기다리기 X
$('Pio*0:neck.angle').d = -10;
```
...
## 목이 움직이는 중인가?
삐오의 목이 움직이고 있는지 아닌지 여부를 **참(1) / 거짓(0)** (으)로 반환합니다.
...
삐오의 눈 LED 색을 색상 카테고리 블록으로 설정합니다.  
왼쪽, 오른쪽 또는 양쪽의 눈 LED 색을 변경할 수 있습니다.
...
// 삐오 오른쪽 눈을 R:255, G:255, B:255로 정하기
$('Pio*0:eye.right.rgb').d = [255, 255, 255];

// 삐오 양쪽 눈을 무작위 색상으로 정하기
...
지정한 R,G,B 값만큼 삐오의 눈 LED 색을 변경합니다.   
...
## 버저음 설정하기
지정된 주파수로 삐오의 버저음을 설정합니다.  
주파수의 범위는 10hz ~ 4200hz 입니다.

![](https://github.com/user-attachments/assets/71843448-be97-40c3-9b17-c5457f1db9eb)
...
## 소리가 재생 중인가?
삐오의 소리가 재생중인지 아닌지 여부를 **참(1) / 거짓(0)** (으)로 반환합니다.
...
### 자바스크립트 코드
```javascript
// 삐오의 소리가 재생 중인가? - 재생시 true, 아닐시 false
...
## 배터리 충전 상태 값
삐오의 배터리 충전 상태 값을 가져옵니다.

![](https://github.com/user-attachments/assets/d6425332-7b2e-4616-8cfc-4f4ab79d9fdd)
