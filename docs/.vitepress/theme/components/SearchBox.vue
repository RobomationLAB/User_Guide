<!-- 기본 테마의 VPLocalSearchBox 사본. config.js 의 vite.resolve.alias 로 교체된다.
     원본과의 차이는 세 곳뿐이다 (검색 로직·하이라이팅·접근성은 그대로):
       1) 검색 결과를 페이지 단위로 묶어 재배열
       2) 페이지 제목을 그룹 헤더로 뽑고, 각 행 브레드크럼에서는 제거
       3) .group-header 스타일 추가
...
debouncedWatch(
  ...
  async ([index, filterTextValue, showDetailedListValue], old, onCleanup) => {
    ...
    // Search
    // 같은 페이지의 결과가 흩어지지 않게 페이지 단위로 묶어 재배열한다.
    // 원본 결과는 점수 내림차순이므로 페이지가 처음 등장한 순서를 그대로 쓰면
    // 관련도가 높은 페이지가 위로 온다.
    const searched = index
      .search(filterTextValue)
      .slice(0, 16) as (SearchResult & Result)[]
    const byPage = new Map<string, (SearchResult & Result)[]>()
    for (const r of searched) {
      const page = r.id.split('#')[0]
      if (!byPage.has(page)) byPage.set(page, [])
      byPage.get(page)!.push(r)
    }
    results.value = [...byPage.values()].flat()
...
// 결과 목록을 [페이지 헤더, 결과, 결과, 페이지 헤더, ...] 로 펼친다.
// selectedIndex 는 계속 results 배열의 인덱스를 가리키므로 키보드 이동 로직은
// 손대지 않아도 된다. 헤더는 선택 대상이 아니다.
type SearchRow =
  | { type: 'header'; key: string; label: string }
  | { type: 'result'; key: string; p: SearchResult & Result; index: number }

const groupedRows = computed<SearchRow[]>(() => {
  const rows: SearchRow[] = []
  let lastPage: string | null = null
  results.value.forEach((p, index) => {
    const page = p.id.split('#')[0]
    if (page !== lastPage) {
      lastPage = page
      // titles[0] 이 페이지 제목이다. 페이지 첫 섹션이면 titles 가 비어 있어
      // title 자체가 페이지 제목이 된다.
      rows.push({ type: 'header', key: 'h:' + page, label: p.titles[0] || p.title })
    }
    rows.push({ type: 'result', key: p.id, p, index })
  })
  return rows
})
      ...
      <div class="shell">
        ...
        >
          <template v-for="row in groupedRows" :key="row.key">
            <!-- 페이지 그룹 헤더. 선택 대상이 아니라 role="presentation" 이다. -->
            <li v-if="row.type === 'header'" class="group-header" role="presentation">
              <span class="group-title" v-html="row.label" />
            </li>

            <li
              v-else
              :id="'localsearch-item-' + row.index"
              :aria-selected="selectedIndex === row.index ? 'true' : 'false'"
              role="option"
            >
              <a
                :href="row.p.id"
                ...
                :aria-label="[...row.p.titles, row.p.title].join(' > ')"
                @mouseenter="!disableMouseOver && (selectedIndex = row.index)"
                @focusin="selectedIndex = row.index"
                @click="$emit('close')"
              ...
              >
                <div>
                  <div class="titles">
                    ...
                    <!-- titles[0] 은 그룹 헤더로 올라갔으므로 여기서는 뺀다 -->
                    <span
                      v-for="(t, i) in row.p.titles.slice(1)"
                      :key="i"
                      class="title"
                    >
                    ...
                    <span class="title main">
                      <span class="text" v-html="row.p.title" />
                    </span>
                  </div>

                  <div v-if="showDetailedList" class="excerpt-wrapper">
                    <div v-if="row.p.text" class="excerpt" inert>
                      <div class="vp-doc" v-html="row.p.text" />
                    </div>
