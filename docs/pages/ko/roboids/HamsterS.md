...
| option | 드롭다운 옵션 | 입출력 모드 | 아날로그 입력(analog_input), 디지털 입력(digital_input), 디지털 입력 풀업(digital_input_pullup), 디지털 입력 풀다운(digital_input_pulldown), 전압 입력(analog_input_voltage), 서보 출력(servo_output), PWM 출력(pwm_output), 디지털 출력(digital_output) | - |
