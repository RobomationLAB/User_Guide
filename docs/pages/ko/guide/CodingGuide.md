...
1. [RobomationLAB 로봇 코딩 교육 플랫폼](#platform)  
    1-1) [로봇 코딩 프로그램](#coding-programs)  
    1-2) [블록 ↔ 파이썬 실시간 양방향 변환](#block-python-conversion)  
    1-3) [코딩 프로그램 주요 특징](#features)  
    1-4) [실시간 로봇 제어 방식](#control-method)  

2. [RobomationLAB 로봇 프로그래밍 방식](#programming-method)  
    2-1) [순차 실행과 병렬 실행](#sequential-parallel)  
    2-2) [setup 함수](#setup)  
    2-3) [loop 함수](#loop)  

3. [RobomationLAB 로봇 코딩 기본 문법 체계](#grammar-syntax)  
    3-1) [robomation 패키지 가져오기](#import-package)  
    3-2) [로봇 인스턴스 생성](#create-instance)  
    3-3) [로봇 제어 메서드](#control-methods)  
    3-4) [센서 및 상태 읽기](#sensor)  
    3-5) [이벤트 감지](#event)  
    3-6) [동작 완료 대기 (wait)](#wait)  
    3-7) [유틸리티 함수 (Utils)](#utils)   

<br><br>

## RobomationLAB 로봇 코딩 교육 플랫폼 {#platform}
...
### 로봇 코딩 프로그램 {#coding-programs}

#### Block Composer (블록 컴포저) {#block-composer}
...
#### Script Composer (스크립트 컴포저) {#script-composer}
**Script Composer는 파이썬 코딩을 통해 자사 로봇들을 쉽고 빠르게 제어하며, 파이썬 문법 및 로봇 코딩의 기초를 학습할 수 있는 도구입니다.**  
...
### 블록 ↔ 파이썬 실시간 양방향 변환 {#block-python-conversion}
RobomationLAB은 블록 코딩 에디터와 파이썬 코딩 에디터를 함께 제공하며, 두 에디터는 실시간으로 양방향 동기화됩니다.
...
> (잘못된 문법이거나 블록으로 변환할 수 없는 코드인 경우 전환이 되지 않을 수 있습니다.  
자세한 내용은 [코딩 규칙](CodingRules) 문서를 참고하세요.)

<br>

<br><br>

## RobomationLAB 로봇 프로그래밍 방식 {#programming-method}

### 순차 실행과 병렬 실행 {#sequential-parallel}
...
( `Utils.wait` 함수에 대해서는 이후 [유틸리티 함수 (Utils)](#utils) 에서 다시 설명합니다. )
...
def loop():
    pass
```
( 로봇 인스턴스 및 메서드 문법에 대해서는 이후 [RobomationLAB 로봇 코딩 기본 문법 체계](#grammar-syntax) 에서 다시 설명합니다. )
...
```
( 이벤트 감지(`tap()`) 등 문법에 대해서는 이후 [RobomationLAB 로봇 코딩 기본 문법 체계](#grammar-syntax) 에서 다시 설명합니다. )  

<br><br>

## RobomationLAB 로봇 코딩 기본 문법 체계 {#grammar-syntax}
RobomationLAB에서 제공하는 로봇 코딩 프로그램에서 코드를 작성할 때 지켜야 할 기본 문법 체계는 다음과 같습니다.  
로봇 제어는 `robomation` 파이썬 패키지를 통해 이루어지며,  
로봇을 인스턴스로 만든 뒤 그 인스턴스의 메서드를 호출하는 방식으로 코드를 작성합니다.

### robomation 패키지 가져오기 {#import-package}
