...
| ID | 항목 | 값 | 단위 | 비고 |
|---|---|---|---|---|
| `BEAGLE.DRIVE.SPEED_RANGE` | 속도 지정 범위 | -32,767 ~ 32,767 | **PPS** | **백분율이 아닙니다** |
...
| `BEAGLE.OUT.BUZZER` | 부저 주파수 | 1 ~ 167,772.15 Hz | `충돌` 본문 참고에는 6,502.3Hz |
| `BEAGLE.OUT.NOTE` | 음정 | 88건반, A0 ~ C8 | `충돌` 사양표에는 A3~A7 |
...
| `BEAGLE.POWER.RUNTIME` | 연속 동작 시간 | `미공개` | 시간 | |
| `BEAGLE.POWER.CHARGE` | 충전 시간 | `미공개` | 시간 | |
...
### R1. 속도는 백분율이 아니라 PPS 다 (MUST)

**비글의 속도 단위는 초당 펄스 수(PPS)입니다.**  
지정 범위는 ±32,767 이지만 **실제로 나오는 값은 약 4,000 까지**입니다.

**안내 문구 예**  
> 비글의 속도 단위는 백분율이 아니라 초당 펄스 수(PPS)입니다.  
> "속도 50" 은 최대의 1% 남짓입니다. 실제 유효 범위는 약 4,000 까지이고,  
> 보통 속도라면 2,000 정도를 사용합니다.
...
| 항목 | 채택값 | 다른 표기 |
|---|---|---|
| 부저 최대 주파수 | 167,772.15 Hz | 본문 참고 6,502.3Hz / 사양표 165kHz |
| 제어 주기 | 20 ms (50Hz) | 사양표 초당 100회(10ms) |
| 음정 범위 | A0 ~ C8 (1~88) | 사양표 A3 ~ A7 |
| 속도 지정 범위 | -32,767 ~ 32,767 | 패킷 표에 -327677 ~ 32768 (오기) |
